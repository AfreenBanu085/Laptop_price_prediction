...
df_path = os.path.join(dir_of_interest,'Tesla_Laptop.csv')

IMAGE_PATH = os.path.join(dir_of_interest, "Elon_musk.jpg")


def main():
    st.title(':green[Hello Everyone !] :wave:')
    
    html_temp = """
    <div style="background-color:#586e75;padding:10px">
    <h2 style="color:white;text-align:center;">Laptop Price Prediction Using Streamlit</h2>
    </div>
    """
    ...
    with st.form('form1'):
        ...
        if submitted:
            ...
            if d == 'DDR4':
                ...
                ram_type = 7
            st.success(f"Price prediction : {int(model.predict([[os_,ram_type,ram,storage]])[0])}")
